...
${CLUSTER_CONFIG_FOR_PINNIPED}-for-pinniped:
	kind create cluster \
		...
		--config=./site/docs/reference/manifests/kubeapps-local-dev-apiserver-no-oidc-config.yaml \
		--retain \
		--wait 10s
	kubectl apply --kubeconfig=${CLUSTER_CONFIG_FOR_PINNIPED} -f ./site/docs/reference/manifests/kubeapps-local-dev-users-rbac.yaml
...
${ADDITIONAL_CLUSTER_CONFIG_FOR_PINNIPED}-for-pinniped: devel/dex.crt-for-pinniped
	kind create cluster \
		...
		--config=./site/docs/reference/manifests/kubeapps-local-dev-additional-apiserver-config-for-pinniped.yaml \
		--retain \
		--wait 10s
	kubectl apply --kubeconfig=${ADDITIONAL_CLUSTER_CONFIG_FOR_PINNIPED} -f ./site/docs/reference/manifests/kubeapps-local-dev-users-rbac.yaml
	kubectl apply --kubeconfig=${ADDITIONAL_CLUSTER_CONFIG_FOR_PINNIPED} -f ./site/docs/reference/manifests/kubeapps-local-dev-namespace-discovery-rbac.yaml
