...
import (
	...
	kappcmdcore "carvel.dev/kapp/pkg/kapp/cmd/core"
...
// downgradableAnnKey is the annotation key that allows a PackageInstall to be downgraded.
// Inlined from carvel.dev/kapp-controller/pkg/packageinstall.DowngradableAnnKey to avoid
// importing the packageinstall package. That package transitively pulls in
// carvel.dev/kapp-controller/pkg/reconciler, which does not compile against
// sigs.k8s.io/controller-runtime v0.23 (its handler.TypedEventHandler now requires two
// type arguments). The constant's value is part of kapp-controller's public API and is
...
func getRepoNameFromAnnotation(repoRefAnnotation string) string {
	...
	// See https://github.com/carvel-dev/kapp-controller/pull/532
